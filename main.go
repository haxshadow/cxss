...
type URLResult struct {
	...
	LinesForFile   []string // simplified lines (no prefix) to save to file
	UnfilteredAdd  int64
	TimeoutsAdd    int64
	ErrorsAdd      int64
}
...
func main() {
	// Flags (in the exact serial order requested)
	flag.Usage = func() {
		printBanner()
	...
	// prepare optional output file (separate from stdout)
	...
	// collect URLs from -u or -f (file or stdin)
	urls := make([]string, 0)
	if flagURL != "" {
	...
	// fallback: read from stdin if no URLs provided and stdin is piped
	if len(urls) == 0 {
	...
	// per-index channels to enable immediate printing in serial order:
	resultChans := make([]chan URLResult, total)
	for i := 0; i < total; i++ {
		resultChans[i] = make(chan URLResult, 1)
	}
	...
	for i, u := range urls {
		...
		go func(idx int, target string) {
			...
			// send to per-index channel (buffered)
			resultChans[idx] <- res
		}(i, u)
	}

	// goroutine to close channels when workers done
	go func() {
		wg.Wait()
		for i := 0; i < total; i++ {
			close(resultChans[i])
	...
	// printer: iterate indices in order, print as soon as channel yields (blocks until available)
	for i := 0; i < total; i++ {
		res, ok := <-resultChans[i]
		if !ok {
			continue
		}
		// print lines depending on silent mode
		if flagSilent {
			// show only simplified lines (no prefix)
			for _, lf := range res.LinesForFile {
				fmt.Fprintln(os.Stdout, lf)
			}
		} else {
			// show full lines with prefix
			for _, line := range res.Lines {
				fmt.Fprintln(os.Stdout, line)
			}
		}

		// write simplified lines (no prefix) to file if requested
		if fileWriter != nil && len(res.LinesForFile) > 0 {
			for _, lf := range res.LinesForFile {
				// write with newline
				_, _ = io.WriteString(fileWriter, lf+"\n")
			}
		}
...
func scanSingleURL(idx, total int, targetURL, placeholder, injection string) URLResult {
	...
	for _, param := range paramsToCheck {
		...
		if len(foundChars) > 0 {
			...
			// simplified line for file: remove prefix
